...
static float3 g_ParticleSidePos[4] =
{
    {0.f, 0.5f, -0.5f},
    {0.f, 0.5f, 0.5f},
    {0.f, -0.5f, -0.5f},
...
static float3 g_ParticleUpperPos[4] =
{
    {0.5f, 0.f, -0.5f},
    {0.5f, 0.f, 0.5f},
    {-0.5f, 0.f, -0.5f},
...
void ParticleGS(point VS_OUTPUT_PARTICLE input[1], inout TriangleStream<GS_OUTPUT_PARTICLE> output)
{
    ...
    float3 camDir[4];
    if (g_ParticleShare[0].CamDir == 0)
    {
        camDir = g_ParticleLocalPos;
    }
    else  if (g_ParticleShare[0].CamDir == 1)
    {
        camDir = g_ParticleSidePos;
    }
    else
    {
        camDir = g_ParticleUpperPos;
    }
    for (int i = 0; i < 4; ++i)
    {
        float3  ViewPos = mul(float4(g_ParticleArray[InstanceID].WorldPos, 1.f), g_matView).xyz;
        // 뷰 공간에서의 카메라의 축은 월드축과 동일하다.
        ViewPos = ViewPos + camDir[i] * Scale * 0.5f;
...
float4 ParticlePS(GS_OUTPUT_PARTICLE input) : SV_TARGET
{
    ...
    Color.a = TextureColor.a * g_MtrlOpacity * Alpha * input.Color.a;
    return Color;
}
