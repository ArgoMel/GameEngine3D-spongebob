...
bool CUITextWindow::Init()
{
	...
	m_TextInput = CreateWidget<CEditorInputText>("텍스트", 300.f, 100.f);
	...
	m_Slider->SetText("색상");
	m_FontSizeInput = CreateWidget<CEditorInputText>("폰트 크기", 80.f, 30.f);
...
//shadow
	...
	m_ShadowOffsetX = CreateWidget<CEditorInputText>("X Offset", 80.f, 30.f);
	m_ShadowOffsetX->SetInputType(EImGuiInputType::Float);
	line = CreateWidget<CEditorSameLine>("Line");
	m_ShadowOffsetY = CreateWidget<CEditorInputText>("Y Offset", 80.f, 30.f);
...
void CUITextWindow::TextChangeCallback()
{
	...
	if (resourceWindow&& resourceWindow->GetSelectFont())
	{
		m_SelectWidget->SetFont(resourceWindow->GetSelectFont());
	}
	else if(m_FontItem!="")
	{
		m_SelectWidget->SetFont(m_FontItem);
	}
	m_SelectWidget->SetText(m_TextInput->GetTText());
