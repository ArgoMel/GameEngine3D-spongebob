/*
    공유폴더의 있는 텍스쳐의 이름 끝마다 키워드가 붙어있다
    키워드마다 용도가 다르다
    bml base텍스쳐
    xm specular텍스쳐
*/
...
SkinningInfo SkinningInstancing(float3 Pos, float3 Normal, float3 Tangent, float3 Binormal, float4 BlendWeight, 
    float4 BlendIndex, int InstanceID)
{
    SkinningInfo    Info = (SkinningInfo)0;
    for (int i = 0; i < 4; ++i)
    {
...
VS_OUTPUT_INSTANCING3D MeshInstancingVS(VS_INPUT_INSTANCING3D input)
{
    ...
    {
        Info = SkinningInstancing(input.Pos, input.Normal, input.Tangent, 
...
PS_OUTPUT_GBUFFER MeshInstancingPS(VS_OUTPUT_INSTANCING3D input)
{
    ...
    output.GBuffer2.rgb = ComputeBumpNormalInstancing(input.Tangent, input.Binormal, input.Normal, input.UV, 
        input.InstanceID);
    output.GBuffer2.a = 1.f;
