...
Texture2DMS<float4> g_LightDiffuseTex : register(t18);	//난반사
Texture2DMS<float4> g_LightSpecularTex : register(t19);	//환경광 빛이닿지않는 책상아래가 보인느 정도
Texture2DMS<float4> g_LightEmissiveTex : register(t20);	//정반사
...
LightResult ComputeLight(float3 ViewPos, float3 ViewNormal, float SpecularPower, uint InstanceID, float4 GBuffer4Color)
{
    ...
    {
        LightDir = -Info.LightDir;  //	방향벡터의 반대방향벡터를 구해서 뷰노말과의 내적을 구한다
    }
    ...
    float   Intensity = max(0.f, dot(ViewNormal, LightDir)) * Info.LightIntensity;
...
PS_OUTPUT_LIGHTACC LightAccPS(VS_OUTPUT_LIGHTACC input)
{
    ...
    // 조명연산을 위한 Normal은 뷰공간에 있으므로 뷰 공간의 위치를 구해준다.
    float3 ViewPos;
...
PS_OUTPUT_SINGLE ScreenPS(VS_OUTPUT_SCREEN input)
{
    ...
    output.Color.rgb = GBuffer1Color.rgb * LightDiffuse.rgb + GBuffer1Color.rgb * LightSpecular.rgb 
